...
void checkbounds(ConvexHull &A)
{
	...
	float bounds = 1.0f;
	//Comment this section, if you wish to have the ball bounce inside the cube without screen wrapping, commment the section below
	if (A.origin.x > 1.0f)
		A.origin.x = -1.0f;
	if (A.origin.x < -1.2f)
		A.origin.x = 1.0f;
	// Comment this section if you wish for the object to wrap around the screen, Uncomment the above section
...
=======================================================================================================*/
		// Calculate the reaction force expereienced for the object ( in our case the reaction force is the impulse)
		glm::vec3 reaction_force = j * (-n);

		// Calculate the force exerted by the rolling friction
		glm::vec3 rollingFrictionForce = A.Crr * reaction_force;								// F = Crr x N


		//Since this is a 3D demonstration, the rolling can occur along any of the axis, we need to find the direction in which the rolling friction will take effect.
		// To do this, we take the linear velocity and cross product it with the reaction force, we end up with the direction the rolling force is supposed to oppose.
		glm::vec3 linearDirection;
		linearDirection = glm::normalize(A.linearVelocity);

		//This if/else loop is mainy for error detection, as we should not attemp to normalize a 0 vector, as it leads to #INF values.
		if (glm::length(A.linearVelocity) < FLT_EPSILON)
			linearDirection = glm::vec3(0.0f);

		//Calculate the torque caused by the force experienced due to rolling friction.
		glm::vec3 rollingFrictionTorque = glm::cross(rollingFrictionForce, linearDirection) * glm::length(Ra);

		//Ensure that the rolling friction is opposing the current rotation.
		if (glm::dot(rollingFrictionTorque, A.angularVelocity) > 0.0f)
			rollingFrictionTorque *= -1.0f;

		A.angularVelocity += rollingFrictionTorque;

		//Repeat the above steps for the second object
		reaction_force = j * (n);

		rollingFrictionForce = B.Crr * reaction_force;
		linearDirection = glm::normalize(B.linearVelocity);

		if (glm::length(B.linearVelocity) < FLT_EPSILON)
			linearDirection = glm::vec3(0.0f);

		rollingFrictionTorque = glm::cross(rollingFrictionForce, linearDirection) * glm::length(Rb);

		if (glm::dot(rollingFrictionTorque, B.angularVelocity) > 0.0f)
			rollingFrictionTorque *= -1.0f;

		B.angularVelocity += rollingFrictionTorque;
/*=====================================================================================================
								ROLLING FRICTION RESOLVED								( at this point all the changes caused by the rolling frictional force have been accounted for)
=======================================================================================================*/
...
void update(float t)
{
	...
	if (returnMTV(*boxInfocus, *boxOutfocus, n, Overlap))					// Check if the collision occours
	{
		...
		POC = box1.origin + radius * n;										// Since we are dealing with a sphere, this formula will give us the most accurate point of collision.
...
void main()
{
	...
	window = glfwCreateWindow(800, 800, "3D Rolling Resistance", nullptr, nullptr);
